...
const getCategory = () => {
  getData("https://openapi.programming-hero.com/api/categories").then(
    (categories) => showCategory(categories.categories)
  );
};
...
const showTreeDetails = (id) => {
  ...
  getData(`https://openapi.programming-hero.com/api/plant/${id}`).then(
    (plant) => {
...
cartItemWrapper.addEventListener("click", (e) => {
  if (e.target.id === "cross-icon") {
    ...
    const removedItemPrice = e.target.parentNode.parentNode.querySelector(
